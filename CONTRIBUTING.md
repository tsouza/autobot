...
- **Paths:** a pull request changes only its task's allowed paths. Every task may also edit its crate's `Cargo.toml`, `Cargo.lock`, the `[workspace.dependencies]` entries it needs, the `mod` line in the parent module of a file it adds, its own entry in `autobot_controllers::all_controllers()`, and its own registration line in `crates/autobot-testkit/src/registry/installed.rs` and the adapter it registers. A fixture task may also declare its group's ports in `crates/autobot-testkit/src/registry/<group>.rs` and the `pub mod` line for it in `registry/mod.rs`. Under `crates/*/tests/g_*/` an implementation task may only delete the `#[ignore = "awaiting #N"]` lines that name its own issue.
