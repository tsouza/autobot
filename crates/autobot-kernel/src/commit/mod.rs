...
//!   [`DomainOutcome::Uncertain`], which only the command's receipt resolves.
//!
...
//!   [`domain_digest`](crate::digest::domain_digest) of the status. The
//!   [`TransitionReceipt`](crate::reducer::TransitionReceipt) a commit returns records the
//!   digests its reducer state gives through [`ReducerState`](crate::reducer::ReducerState).
//!   The design names no relation between the two, and nothing here compares them.
//! - A domain commit always pins its `state_revision`. `AcceptDispatch`, the one command that
//!   pins register values instead (KERNEL §3.2), commits through the store's
//!   [`Pin::Current`](crate::store::Pin::Current); a second acceptance of one permit is open in
//!   #298.
...
//!   by the command's receipt (KERNEL §2).
//!
