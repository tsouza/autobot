...
| **Authenticated human principals** | Requesting holds, pauses, reviews, plan revisions, adjudications and kill switches through commands and Interventions, under the roles `WorkContext.spec` names; accepting proposals through the CLI — a new context's configuration by a bootstrap administrator the installation names, charter revisions and plan proposals by the context's reviser (ONBOARD §1). | Editing protected status. A human acts through the same command path as everything else. |
...
- **The intake client.** The process a person runs to onboard a project (ONBOARD §1) is an untrusted proposer, whatever agent drives it. It authenticates as the **intake-submitter identity** of one namespace — the installation's intake namespace, or a context's — a dedicated principal that may create and revise intake kinds in proposal state in that namespace and read them, and nothing else. Every `Intake` in the intake namespace is open to that namespace's identity until it is bound; what the identity writes there still grants nothing. Every submission records its authenticated principal and the client name it claims. What it relays — repository content, issue and pull-request text, the person's interview answers — is untrusted content with provenance, never authority, and a person's answer relayed by it is no act of that person.
