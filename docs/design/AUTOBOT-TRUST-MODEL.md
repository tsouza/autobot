...
| **Admission and RBAC** | Rejecting direct writes to protected status; rejecting a CAS that touches both commit lanes (KERNEL §1); rejecting cross-context references; confining the intake-submitter identity to intake kinds in proposal state; refusing every accept command from a principal that is not human. | Side effects of any kind. Admission validates, it never acts. |
...
- **The intake client.** The process a person runs to onboard a project (ONBOARD §1) is an untrusted proposer, whatever agent drives it. It authenticates as the **intake-submitter identity**: a dedicated principal that may create and revise intake kinds in proposal state, in the installation's intake namespace and in the namespaces of the contexts it serves, and read their status, and nothing else. Every submission records its authenticated principal and the client name it claims. What it relays — repository content, issue and pull-request text, the person's interview answers — is untrusted content with provenance, never authority, and a person's answer relayed by it is no act of that person.
