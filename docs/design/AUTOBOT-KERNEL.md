...
**Unsent operations.** An operation is **unsent** while it is `REQUESTED` with no `send_attempt`, which by the re-request rule above means none for its current `attempt_index`. An unsent operation named by no ledger entry and by no `ISSUED` or `BROKER_ACCEPTED` permit ends without a send in two cases:

- It is **released**, `REQUESTED → RELEASED`, when it is an operation of a TaskRun (§6) whose `Task` is terminal, or for a planning TaskRun whose `Plan` or `Intake` is, so that no attempt will send it, and it is proven not applied. For an operation of a restored ambiguous operation set (§7), the proof is a provider lookup under its `operation_key`, taken while its `RestoreRequest` is `FENCED`, `RECONCILED` or `MAPPED`. For any other, its own record is the proof: nothing is sent without a durable `send_attempt` (step 2), and every entry of its `prior_send_attempts` was moved there by a re-request, which follows only proven non-application.
...
ToolInvocation       PERMITTED → REQUESTED                          (permit invalidated: before acceptance, or on a currency or register re-validation failure or a blocked-target refusal at 2a, before any send)
                     ...
                     REQUESTED → RELEASED                           (unsent, its task terminal, proven not applied, §3.3)
                     REQUESTED → CONFIRMED                          (restored, unsent, a lookup finds it applied, §3.3)
                     REQUESTED → UNRESOLVED                         (restored, unsent, a provider offering neither idempotency nor lookup, §7)
