...
**Unsent operations.** An operation in `REQUESTED` with no `send_attempt` for its current `attempt_index`, named by no ledger entry and by no `ISSUED` or `BROKER_ACCEPTED` permit, ends without a send in two cases:

- It is **released**, `REQUESTED → RELEASED`, when it is an operation of a TaskRun (§6) whose `Task` is terminal, or for a planning TaskRun whose `Plan` or `Intake` is, so that no attempt will send it, and it is proven not applied. For an operation of a restored ambiguous operation set (§7), the proof is a provider lookup under its `operation_key`, taken once the old installation is fenced. For any other, its own record is the proof: nothing is sent without a durable `send_attempt` (step 2), and a re-request follows only proven non-application.
- A restored one is **confirmed**, `REQUESTED → CONFIRMED`, whatever its carrier, when such a lookup finds it applied; its `EffectReceipt` records the remote identity the lookup returned as the evidence.
...
ToolInvocation       PERMITTED → REQUESTED                          (permit invalidated: before acceptance, or on a currency or register re-validation failure or a blocked-target refusal at 2a, before any send)
                     ...
                     REQUESTED → RELEASED                           (no send_attempt, its task terminal, proven not applied, §3.3)
                     REQUESTED → CONFIRMED                          (restored, no send_attempt, a lookup finds it applied, §3.3)
                     REQUESTED → UNRESOLVED                         (restored, no send_attempt, a provider offering neither idempotency nor lookup, §7)
