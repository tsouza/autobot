...
**Blocked targets.** An operation is **unsettled** from its `OUTCOME_UNKNOWN`, or for a restored operation escalated from `REQUESTED` from the pair added before that escalation (§7), until it reaches a terminal state: while it is `OUTCOME_UNKNOWN`, `RECONCILING` or `UNRESOLVED`, and through a re-request after proven non-application. The `blocked_targets` register is the list, in the order the pairs were added, of one pair `(target_identity, operation_uid)` per unsettled operation. A pair is added at step 4, or before that escalation, and removed by a reconciliation request from the broker once the operation's terminal state is durable and its entry is gone; a request naming an absent pair is a no-op. Capacity is reserved at acceptance: pairs and ledger entries share the ledger's capacity (§3.2), so adding a pair never waits; only recovery row 6, adding the pair of an operation whose entry was lost, and that escalation can take the list past the bound, and acceptance is refused until it drops back.
...
**Reconciliation.** An operation in `RECONCILING` may be re-requested (same `operation_key`, `attempt_index + 1`) only when the adapter proves definitive non-application, and only once its permit is `CONSUMED`. The proofs are a provider lookup, provider deduplication, and a rate-limit answer from a provider whose declared capability marks its rate-limit answers authoritative; after such an answer the re-request is sent no earlier than the provider's back-off allows. The re-request is one CAS that also appends the operation's `send_attempt` to its append-only `prior_send_attempts` and clears `send_attempt`, so `send_attempt` always belongs to the current `attempt_index` and every guard that reads it reads the current attempt; `prior_send_attempts` keeps the earlier ones. A negative search proves nothing, and neither does a rate-limit answer from any other provider. When the provider's declared capability offers neither idempotency nor lookup, the operation becomes `UNRESOLVED` after `provider_reconcile_bound`: retention pinned, its dependents still blocked, plan completion blocked, an `Intervention` for human adjudication created. `UNRESOLVED` is never garbage-collected, never re-requested and never counted as success or failure. A provider whose declared capability lacks a required semantic yields `BLOCKED_UNSUPPORTED` before any send.

**Adjudication.** An `UNRESOLVED` operation ends only through one human adjudication, recorded in its `adjudication` field, a domain field of the `ExternalOperation`: the `Decision` it references and the outcome chosen, `CONFIRMED`, `FAILED` or `COMPENSATED`, or, for an operation with no `send_attempt`, `RELEASED`, the finding that it was not applied. The broker writes it once, by a domain CAS that also moves the operation from `UNRESOLVED` to the chosen outcome; its preconditions are `state = UNRESOLVED` and the field absent. A second adjudication, or a delayed copy of the first, can therefore never reopen or replace it, and no crash can leave an adjudication recorded on an operation that has not yet ended. Once that CAS is durable, the broker releases the operation's pair. `COMPENSATED` records the adjudicator's evidence that the effect was compensated outside AutoBot; AutoBot sends no compensating effect, and automatic compensation is DEFERRED to G-FORGE. A `COMPENSATED` operation counts as a failure wherever outcomes are counted.

**Unsent operations.** An operation is **unsent** while it is `REQUESTED`. A `REQUESTED` or `PERMITTED` operation carries no `send_attempt`, since only step 2b or recovery row 4 writes one, in the CAS that moves it to `DISPATCHING`, and the re-request clears it; an unsent operation therefore has none for its current `attempt_index`.

An operation's **carrier** is the aggregate whose domain commit carried its intent, the `aggregate_uid` its `operation_key` is derived from, recorded on its `EffectIntent`. A later domain commit of the carrier **withdraws** the intent, whether or not it carries another in its place, when its command names the intent's `operation_key` in its input, so that its receipt, and every receipt rebuilt from its `PREPARED` receipt (§1), lists the key in `withdrawn_intents`. The broker requests no permit for an operation whose intent is withdrawn; a withdrawal recalls nothing already accepted or sent, which proceeds under this section.

An unsent operation named by no ledger entry and by no `ISSUED` or `BROKER_ACCEPTED` permit ends without a send in two cases:

- It is **released**, `REQUESTED → RELEASED`, when no attempt will send it and it is proven not applied. No attempt will send an operation whose intent its carrier has withdrawn, nor an operation of a TaskRun (§6) whose `Task` is terminal, or for a planning TaskRun whose `Plan` or `Intake` is. For an operation of a restored ambiguous operation set (§7), the proof is a provider lookup under its `operation_key`, taken while its `RestoreRequest` is `FENCED`, `RECONCILED` or `MAPPED`. For any other, its own record is the proof: nothing is sent without a durable `send_attempt` (step 2), and every entry of its `prior_send_attempts` was moved there by a re-request, which follows only proven non-application.
- A restored one is **confirmed**, `REQUESTED → CONFIRMED`, whatever its carrier, when such a lookup finds it applied; its `EffectReceipt` records the remote identity the lookup returned as the evidence. It counts as a success, like any `CONFIRMED` operation: its `EFFECT` reservation, if it holds one, follows the §10 `BudgetReservation` rule for a terminal consumer, `UNKNOWN` until its usage settles and then `COMMITTED`, or `EXPIRED` by explicit conservative expiry, and the broker's `UsageReceipt` for the TaskRun's operations includes the effect (§8).

`RELEASED` is terminal and means proven not applied: the operation's intent is `ACKNOWLEDGED`, its `EFFECT` reservation, if it holds one, is `RELEASED` as proven zero use, and it counts as neither success nor failure; this holds as well for an `UNRESOLVED` operation adjudicated `RELEASED`. After either case the broker releases the pair a re-request left on the operation, if any. The broker submits `AcceptDispatch` only for a permit its operation records as `PERMITTED`, and an ended operation records none, so a permit whose create lands after the operation ended is never accepted and ends `INVALIDATED` or `EXPIRED` (§3.2). An operation in `PERMITTED` reaches `REQUESTED` first, through its permit's invalidation or expiry or, from `ACCEPTED_NOT_SENT`, through a currency failure at step 2 or recovery row 1.
...
Every AgentRun receives its own `ExecutionIdentity` `(task_run, workspace, agent_run, execution_epoch, installation_id, audience)` and its own short-lived `CredentialGrant` bound to that identity, its audience, repositories, paths, operations, installation lineage, execution epoch, expiry and revocation generation. Two AgentRuns never share an identity or a grant (the sessions of one AgentRun, its continuations, keep its own, §9), and a grant carries only what its session's role may request (ROLES §4); `workspace` is empty for a session that holds no workspace. Every session runs in a `TaskRun`: a task's sessions in that task's TaskRun, and the Manager's, the Integrator's and those that assess a plan's integrated candidate in a **planning TaskRun**, whose spec names the `Plan` or `Intake` it serves in place of a task and which holds no workspace (ROLES §4). An `Intake` names its own `Budget`, and a planning TaskRun that serves an `Intake` takes its `ATTEMPT` reservation, and the `EFFECT` reservation of each of its operations, from that `Budget`. One that serves an `Intake` of the installation's intake namespace (ONBOARD §1 step 4), where no `WorkContext` exists yet, has that `Intake` in the `WorkContext`'s place in the broker's `WorkContext` check, and sends no effect, since acceptance is a `WorkContext` commit (§3.2). Agents never hold a reusable credential. The broker validates namespace, WorkContext UID, target UID, audience, repository, path, operation, lineage, epoch, revocation generation and expiry on every privileged call; a grant that fails any of these, or references another context, is refused.
...
Custody checkpoints recur. While a run uses its workspace, one is taken at the checkpoint cadence its `CustodyPolicy` declares (the profile's, M0 §2) and one at run end. Each moves the workspace `IN_USE → PRESERVING`, which is when the write fence is confirmed, and then `PRESERVED`; a run that goes on working lifts the fence with `PRESERVED → IN_USE`. The write fence holds for as long as the workspace is `PRESERVED`, so a `PRESERVED` workspace holds nothing its last verified checkpoint lacks, and retirement, which requires `PRESERVED`, never loses a write made after that checkpoint. A custody checkpoint records, when it begins, the digest, over its fields, of every `CREATED` `AgentCheckpoint` of a session holding its workspace: a session whose `ExecutionIdentity` names the workspace, whether it is running or has ended and awaits its continuation. An `AgentCheckpoint` is **covered** by the first `VERIFIED` custody checkpoint that recorded its digest, so coverage is ordered by that record alone and needs no order between the two aggregates, and it is `VERIFIED` only once it is covered, so a failed custody checkpoint only defers it to the next; the checkpoint of a session that holds no workspace is verified without one. "The next checkpoint" of I-5 is the next custody checkpoint of the workspace, and so is every scope diff "at every custody checkpoint" (ROLES §2).
...
**The read-only guard.** A restore writes every restored `WorkContext` with `dispatch_authority_generation` holding no value, and only `AdvanceDispatchAuthorityGeneration` gives it one; the Custody controller submits it only from `MAPPED`, with the receipt above, and moves the `RestoreRequest` to `DISPATCH_ENABLED` on its `COMMITTED` receipt. A register with no value matches no pin. Until then three checks refuse: `AcceptDispatch` fails its `dispatch_authority_generation` precondition for every permit (§3.2); `RecoverAcceptedDispatch` row 1 fails its register re-validation, so a restored `ACCEPTED_NOT_SENT` entry is released instead of sent (§3.3); and the broker issues no `CredentialGrant`, and honours none on a privileged call, for an installation whose `RestoreRequest` is not `DISPATCH_ENABLED`. A restored operation of the ambiguous set that is unsent (§3.3) is sent after the barrier only once a provider lookup under its `operation_key` proves it was not applied; the broker requests no permit for it before then. A deduplication answer is no proof here, since obtaining one takes a send. On a provider whose declared capability offers no lookup, whether or not it offers idempotency, it is left for human adjudication: while its `RestoreRequest` is `FENCED`, `RECONCILED` or `MAPPED`, the broker adds its pair, as at step 4, and then moves it from `REQUESTED` to `UNRESOLVED`, which only an adjudication ends (§3.3). A lookup that finds it applied, or its release, ends it instead (§3.3, unsent operations). Old external-operation identities (`operation_key`) are preserved across restore; old pending actions are never replayed merely because a recovered resource lists them. Witness or broker uncertainty fails closed.

## 8. The canonical-record obligation — I-9

`OutcomeRecord`, `UsageReceipt` and `TelemetryGap` are aggregates created through the create protocol of §2, not telemetry. Their durability under API unavailability is an obligation on the `TaskRun`, a planning TaskRun included. Its entries are one `outcome` entry for the run and a `usage` entry for each **usage producer** that spends, committed before it spends (step 1). The usage producers are each session of each of its AgentRuns, keyed `(agent_run, session_sequence)`, and the broker, for the effects of the TaskRun's operations: the sends it makes under the TaskRun, an inherited operation's included (§9), and an unsent restored operation confirmed applied (§3.3).

1. At admission, `TaskRun.status.expected_records = {outcome: PENDING, usage: {}}` is set. Before a usage producer spends, its entry `expected_records.usage[producer] := PENDING` commits on the `TaskRun`: a session's before its first model call, which its runtime adapter makes only after that commit, and the broker's before it accepts the first effect of the TaskRun's operations or confirms an unsent restored one (§3.3). A producer with no entry never spends, so no spend is outside the obligation. Each entry's `record_deadline` is the profile's `record_deadline` bound (FORMAL §6) after the later of the TaskRun's terminal commit, which sets it for every entry then present, and the entry's creation. No entry therefore gaps while its run is not terminal, and `outcome` never gaps before verification ends.
2. The producing process — each session's runtime adapter, the broker, or for the outcome the verifier (the TaskRun controller for a planning TaskRun) — writes its record first to its **durable local outbox** (inside the checkpointed workspace area, or the broker's persistent queue, which a session with no workspace uses) with its deterministic name, input digest and create key, *before* reporting completion. A usage producer writes one `UsageReceipt` for its spend under the TaskRun, naming itself. A `UsageReceipt` the outbox refuses (`OutboxRefused`) is written again by a later session of the same `AgentRun` (ROLES §4), under that session's own producer key, as a **replacing receipt**, whose producer is a session of the refused entry's `AgentRun` with a greater `session_sequence`: one per refused entry, naming that entry's producer as the one it replaces, with that name in its deterministic name and create key, and recording only that entry. A session's own spend is never on a replacing receipt.
...
4. An entry becomes `RECORDED` only on its own record's `COMMITTED` create receipt: `outcome` on the `OutcomeRecord`'s, `usage[producer]` on the receipt that producer wrote for its own spend, which replaces nothing, or on a replacing receipt that names it (step 2).
5. An entry still `PENDING` at its `record_deadline` receives a `TelemetryGap` (`gap_kind ∈ {OUTCOME_MISSING, USAGE_MISSING}`) linked to the TaskRun, which for `USAGE_MISSING` names the entry's producer, and the entry becomes `GAP(uid)`. A gap is a canonical record: whatever counts outcomes or costs counts it, as unknown outcome or censored cost, never as absent. A record that commits after its gap closes the gap the entry names (`CLOSED`, linked to the record), and every count uses the record from then on, as an append-only correction; the entry stays `GAP` (§10).
...
ToolInvocation       PERMITTED → REQUESTED                          (permit invalidated: before acceptance, or on a currency or register re-validation failure or a blocked-target refusal at 2a, before any send)
                     ...
                     RECONCILING → REQUESTED                        (non-application proven; same key, attempt_index+1, send_attempt moved to prior_send_attempts)
                     RECONCILING → UNRESOLVED → CONFIRMED | COMPENSATED | FAILED   (human adjudication only)
                     UNRESOLVED → RELEASED                          (human adjudication only: no send_attempt, not applied, §3.3)
                     REQUESTED | PERMITTED → BLOCKED_UNSUPPORTED
                     REQUESTED → RELEASED                           (unsent, no attempt will send it, proven not applied, §3.3)
                     REQUESTED → CONFIRMED                          (restored, unsent, a lookup finds it applied, §3.3)
                     REQUESTED → UNRESOLVED                         (restored, unsent, a provider offering no lookup, §7)
...
expected record      PENDING → RECORDED | GAP   (TaskRun.status.expected_records.outcome, set PENDING at admission, and each
                      expected_records.usage[producer], committed PENDING before its producer spends, §8; GAP is final: a
                      record committed after the gap closes the TelemetryGap and leaves the entry GAP)
...
BudgetReservation    RESERVED → COMMITTED | RELEASED | UNKNOWN | EXPIRED
                     ...
                     (UNKNOWN: its consumer, the TaskRun for ATTEMPT, is terminal or fenced, or the operation for EFFECT
                      is terminal or UNRESOLVED, and its usage is not SETTLED; an EFFECT reservation stays RESERVED while
                      its operation is OUTCOME_UNKNOWN or RECONCILING, so a re-request, or a later attempt that inherits
                      the operation, sends under it, §9; COMMITTED on settlement; RELEASED on proven zero use;
