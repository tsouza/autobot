...
**Unsent operations.** An operation is **unsent** while it is `REQUESTED` with no `send_attempt` for its current `attempt_index`; an earlier attempt's record does not count. An unsent operation named by no ledger entry and by no `ISSUED` or `BROKER_ACCEPTED` permit ends without a send in two cases:

- It is **released**, `REQUESTED → RELEASED`, when it is an operation of a TaskRun (§6) whose `Task` is terminal, or for a planning TaskRun whose `Plan` or `Intake` is, so that no attempt will send it, and it is proven not applied. For an operation of a restored ambiguous operation set (§7), the proof is a provider lookup under its `operation_key`, taken while its `RestoreRequest` is `FENCED`, `RECONCILED` or `MAPPED`. For any other, its own record is the proof: nothing is sent without a durable `send_attempt` (step 2), and a re-request follows only proven non-application.
