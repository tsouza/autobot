...
`RELEASED` is terminal and means proven not applied: the operation's intent is `ACKNOWLEDGED`, its `EFFECT` reservation, if it holds one, is `RELEASED` as proven zero use, and it counts as neither success nor failure. After either case the broker releases the pair a re-request left on the operation, if any. The broker submits `AcceptDispatch` only for a permit its operation records as `PERMITTED`, and an ended operation records none, so a permit whose create lands after the operation ended is never accepted and expires. An operation in `PERMITTED` reaches `REQUESTED` first, through its permit's invalidation or expiry or, from `ACCEPTED_NOT_SENT`, through a currency failure at step 2 or recovery row 1.
