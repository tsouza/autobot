...
WorkContextRegisters = [hold_state, hold_generation,
                        ...
                        blocked_targets,         \* list of [target_identity, operation_uid, adjudication_uid] in order added, one per unsettled operation
...
MaterializeEffectIntent     from a committed receipt of any carrier, the AgentRun for a ToolInvocation · QuarantineEffectIntentCollision
...
RecordSendAttempt           2a ledger SEND_ATTEMPTED, precondition no blocked_targets pair on the target ∨ the first pair on it is the operation's own, or that of the operation it compensates recording the adjudication it names, a refusal handled as a currency failure; then 2b operation DISPATCHING + send_attempt
RemoteSend · ObserveExternalOutcome
...
ReconcileAmbiguousEffect · EscalateUnresolvedOperation
AdjudicateUnresolvedOperation   Broker; first a reconciliation-only CAS setting the pair's adjudication_uid, a no-op unless it is NONE; then only for the adjudication the pair holds: UNRESOLVED → CONFIRMED | FAILED and ReleaseBlockedTarget, or compensation chosen; every other adjudication rejected
...
CommitCompensatingIntent    the originating command's owner; a domain commit, for an adjudication choosing compensation that the operation's pair holds, carrying an intent whose desired_outcome names the operation and the adjudication
RecordCompensated           Broker; UNRESOLVED → COMPENSATED; precondition the pair holds an adjudication choosing compensation ∧ its compensating operation CONFIRMED; a compensating operation ending otherwise clears the pair's adjudication_uid
