...
CommandReceipt       = [uid, command_uid, idempotency_key, target_kind, target_uid, parent_context_uid,
                        ...
                        withdrawn_intents,       \* set of operation_key of intents of earlier domain commits of the target that this commit withdraws, from the command's input (KERNEL §3.3); empty unless it withdraws one
...
ExternalOperation    = [uid, operation_key, attempt_index, provider, remote_identity, source_head,
                        ...
                        adjudication,            \* NONE | [decision_uid, outcome ∈ {CONFIRMED, FAILED, COMPENSATED, RELEASED}]; written once, by a human adjudication of an UNRESOLVED operation
                        state]
...
IssueAdmissionStamp         create on the broker's command, client request key (operation_uid, operation state_revision), for an operation with Idle(op) ∧ ¬Withdrawn(op); precondition hold_state = RUNNING ∧ plan revision_phase = ACTIVE ∧ manager phase = ACTIVE ∧ (provider_binding names a merge ⇒ basis_uid ≠ ∅); every register pin from one WorkContext read; expires_at ≤ creation + the profile's replay window, the window every AcceptDispatch pins
...
AcknowledgeDispatch         reconciliation-only; removes the entry only after acceptance ∧ ((a) terminal ∨ OUTCOME_UNKNOWN ∨ (b) Unsent(op) after a currency or register re-validation failure, the entry ACCEPTED_NOT_SENT or SEND_ATTEMPTED) recorded on the operation
...
\* effects
Unsent(op)                  ≜ op.state = REQUESTED; op.send_attempt = NONE then holds (F-18), for the current attempt by construction (ProveNonApplication); the one definition of unsent (KERNEL §3.3)
Idle(op)                    ≜ Unsent(op) ∧ no ledger entry and no ISSUED or BROKER_ACCEPTED permit names op
Withdrawn(op)               ≜ a COMMITTED CommandReceipt of a domain commit on op's carrier, the aggregate_uid of its EffectIntent, at a proposed_revision above that intent's committed_revision, lists op.operation_key in withdrawn_intents (KERNEL §3.3)
Restored(op)                ≜ op ∈ the ambiguous_operation_set of its installation's RestoreLineage
RestoreWindow(op)           ≜ Restored(op) ∧ that lineage FENCED, RECONCILED or MAPPED
RestoreLookup(op)           ≜ a provider lookup under op.operation_key answered while RestoreWindow(op); it proves op applied or not applied
Unprovable(op)              ≜ Idle(op) ∧ RestoreWindow(op) ∧ op's capability offers no lookup, whether or not it offers idempotency (KERNEL §7)
...
AddBlockedTarget            reconciliation-only; appends [target_identity, operation_uid] to blocked_targets; precondition the operation's entry SEND_ATTEMPTED, or recovery row 6, or Unprovable(op); no-op when present
...
RecoverAcceptedDispatch     restart scan; the six-row table, row 4 moving a PERMITTED operation to DISPATCHING with send_attempt in one CAS before acting as row 2; brings every permit level; removes the pair of every terminal operation
...
EscalateUnresolvedOperation Broker; RECONCILING → UNRESOLVED; precondition the capability offers neither idempotency nor lookup ∧ provider_reconcile_bound passed; or REQUESTED → UNRESOLVED; precondition Unprovable(op) ∧ its pair in blocked_targets; either way creates the adjudication Intervention
AdjudicateUnresolvedOperation   Broker, domain, on a human adjudication; one CAS writing adjudication := [decision_uid, outcome] and UNRESOLVED → outcome, outcome ∈ {CONFIRMED, FAILED, COMPENSATED, RELEASED}; precondition state = UNRESOLVED ∧ adjudication = NONE; COMPENSATED only with the adjudicator's evidence of compensation outside AutoBot; RELEASED, the finding that op was not applied, only when op.send_attempt = NONE, and then its EFFECT BudgetReservation, if any, RELEASED; then ReleaseBlockedTarget
...
ReleaseUnsentOperation      Broker; REQUESTED → RELEASED; precondition Idle(op) ∧ (Withdrawn(op) ∨ op is an operation of a TaskRun whose Task, or whose planning_subject, is terminal) ∧ (Restored(op) ⇒ RestoreLookup(op) proves it not applied); then its EFFECT BudgetReservation, if any, RELEASED, and ReleaseBlockedTarget
ConfirmUnsentOperation      Broker; REQUESTED → CONFIRMED; precondition (op is an operation of a TaskRun ⇒ the broker's usage entry of that TaskRun committed, ExpectUsage) ∧ Idle(op) ∧ RestoreLookup(op) proves it applied; its EffectReceipt records the remote_identity the lookup returned; its EFFECT BudgetReservation, if any, UNKNOWN until its usage settles, then COMMITTED, or EXPIRED by explicit conservative expiry, and the broker's UsageReceipt includes the effect; then ReleaseBlockedTarget
BlockUnsupportedOperation   Broker; REQUESTED | PERMITTED → BLOCKED_UNSUPPORTED; precondition the capability lacks a required semantic
