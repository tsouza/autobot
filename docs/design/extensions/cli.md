...
- **Charter edits are new revisions.** The CLI edits a `Charter` or `ProjectCharter` only by proposing a new revision; a human principal accepts it and every entry in it, and the repository copy of a charter is a read-only projection the CLI never reads back as a source.
...
- **The intake is exposed as stages**, never as one `run brief.md`: capture, inspect, answer, scope, propose, accept, activate. A convenience chain may sequence them under policy; each transition stays visible, resumable and auditable.
