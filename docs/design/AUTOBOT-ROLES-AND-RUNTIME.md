...
| **Tester** | Design verification and arrange its execution locally or in CI. | `VerificationRun`s against the exact candidate, including those that run its acceptance suite (§2); findings for failures outside the target. | Rewrite acceptance criteria; delete required checks. |
...
A worker's "blocked" claim — that the assignment cannot be completed inside its capsule and acceptance contract — is an ordinary outcome of its attempt, recorded like any other; it counts against the worker-strategy configuration of the attempt's routing pin only through that attempt's own cost, never as a failure of the configuration.
...
**Acceptance suite.** The capsule's acceptance evidence names the task's **acceptance suite**: the repository paths of the tests and checks its acceptance runs. The suite lies outside the worker's writable mount and is not readable from it: the workspace is materialized without its paths, and no brokered call of a worker's session returns their content. The Tester's `VerificationRun`s run the suite against the pinned candidate. Every path of the suite is outside the capsule, whatever its globs say, and is compared on canonical identity like any other: a worker's write to one is denied before effect, and a candidate whose checkpoint diff creates, changes or deletes any path of its acceptance suite is ineligible for evidence, with the consequences of an out-of-scope mutation above.
...
| Worker | the TaskRun of its task | writes to that TaskRun's workspace inside the capsule, and brokered tool calls inside it; no read of its acceptance suite (§2) |
