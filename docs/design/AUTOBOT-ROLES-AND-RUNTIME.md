...
| Tool-side effects | Forge, CI, deployment and credential effects exist only as brokered `ToolInvocation`s; `effect_kinds` and the repository set are checked by the broker's currency check before the send (KERNEL §3.2). |
...
- `PAUSE` becomes the plan-level `PAUSED` block. A pause requested for a review carries a review reason; a review that needs a full hold is a `HOLD` beside it.
- `QUIESCE` and `SUPERSEDE` drive KERNEL §5: `QuiescePlan`, and for `SUPERSEDE` then `SupersedePlanRevision` once the replacement is `MEMBERS_VERIFIED`.
- `FAIL` ends a plan that is `ACTIVE`, `PAUSED` or `QUIESCING` in `FAILED`, and `CANCEL` ends any plan not yet terminal in `CANCELLED`. The Plan controller first applies `QuiescePlan` when the register holds the plan's revision `ACTIVE`, waits as KERNEL §5 waits for active attempts, and then commits the terminal phase, never while an activation receipt is `UNCERTAIN`; retirement of the plan's register entries follows (KERNEL §3.1).
- `RESUME` names the `Intervention` it answers and lifts only that one. A `RESUME` of a `HOLD` or of a `KILL_SWITCH` becomes `ReleaseHold`, which removes only that cause; the context returns to `RUNNING` through `CompleteHoldRelease` once no cause is left. A `RESUME` of a `PAUSE` moves the plan `PAUSED → ACTIVE`. A `RESUME` of a `QUIESCE` or of a `SUPERSEDE` resumes the same revision with `ResumePlanRevision`.
...
A paused plan leaves `PAUSED` only by `RESUME` (to `ACTIVE`), by `QUIESCE` or `SUPERSEDE` (to `QUIESCING`), by `FAIL` or by `CANCEL`, and never on its own when work becomes eligible again. No action completes a plan: the Plan controller commits `COMPLETED` once every milestone of its active revision is `ACCEPTED` or `SUPERSEDED` and nothing blocks plan completion (KERNEL §3.3).
