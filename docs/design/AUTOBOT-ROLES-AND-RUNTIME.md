...
| **Worker** | Implement one bounded assignment in one workspace. | Brokered tool calls inside its capsule; checkpoints; a candidate change; findings; "blocked". | Fix, refactor or clean up anything outside the capsule; read or change its acceptance suite (§2); retry an effect blind; approve anything. |
...
A worker's "blocked" claim — that the assignment cannot be completed inside its capsule and acceptance contract — is an ordinary outcome of its attempt, whatever phase its TaskRun ends in, and names its blocker in a `Finding`. The claim is no failure of the worker-strategy configuration of the attempt's routing pin, which bears the attempt's own cost. The escalation cost the claim causes, every later attempt of the task, is attributed to that configuration too, unless the Manager's disposition of the `Finding` accepts the blocker as genuine; then it is attributed to the blocker the `Finding` names.
...
**Acceptance suite.** The capsule's acceptance evidence names the task's **acceptance suite**: the repository paths of the tests and checks its acceptance runs, each with the digest of its content at the candidate basis. The suite lies outside the worker's writable mount and is not readable from it: the workspace, its repository object store included, holds no object that any revision of the repository holds at a suite path, and no brokered call of a worker's session returns one; content that a path outside the suite also holds stays readable at that path. Every suite path, and every ancestor directory of one as an entry of its own, is outside the capsule whatever its globs say and is judged on canonical identity like any other path: creating, changing, deleting or renaming one, or replacing one with a file or a link, is denied before effect, and a candidate whose checkpoint diff does so is ineligible for evidence, with the consequences of an out-of-scope mutation above. A Tester's `VerificationRun` of the suite first writes the suite's basis content into the pinned candidate at its paths and records that each suite path resolves, on canonical identity, to content matching its basis digest; a run without that record, or whose record shows a mismatch, satisfies no acceptance.
...
| Worker | the TaskRun of its task | writes to that TaskRun's workspace inside the capsule, and brokered tool calls inside it (§2) |
