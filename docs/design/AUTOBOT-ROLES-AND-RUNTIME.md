...
A worker's "blocked" claim — that the assignment cannot be completed inside its capsule and acceptance contract — is an ordinary outcome of its attempt, whatever phase its TaskRun ends in: it counts against the worker-strategy configuration of the attempt's routing pin only through that attempt's own cost, never as a failure of the configuration, and the cost of any later attempt of the task is that later attempt's own.
...
**Acceptance suite.** The capsule's acceptance evidence names the task's **acceptance suite**: the repository paths of the tests and checks its acceptance runs, whose content is that of the candidate basis. The suite lies outside the worker's writable mount and is not readable from it: the workspace, its repository object store included, holds no object of the suite's paths, and no brokered call of a worker's session returns their content. The pinned candidate carries those paths exactly as at its candidate basis, and the Tester's `VerificationRun`s run the suite against it. Every path of the suite is outside the capsule, whatever its globs say, so a candidate whose checkpoint diff creates, changes or deletes any of them is ineligible for evidence, with the consequences of an out-of-scope mutation above.
