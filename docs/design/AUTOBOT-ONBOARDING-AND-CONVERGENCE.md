...
Onboarding goes through an **intake client**: a process a person runs, outside AutoBot, in the working directory of the project to onboard (TRUST). For a new project that directory holds only design documents; for an existing one it is a work-in-progress checkout with a forge repository, issues and pull requests. Both cases take the one path below and differ only in what the directory holds. The client analyses the directory, interviews the person and submits the result under the **intake-submitter identity**, which creates and revises intake kinds in proposal state and reads their status, and nothing else (TRUST). A client proposes; it never accepts, activates or grants anything.

**The attributes are the schema.** Every attribute an onboarding settles is a field of the spec of an intake kind — `WorkBrief`, `Intake`, `Project`, `Repository`, `Charter`, `ProjectCharter`, `PlanProposal` (M0 §1) — and a proposal is complete exactly when it validates against the schemas of those specs. Every attribute is a **sourced value**: the value with its **provenance**, one or more entries, each naming a source — a path or URL with a content digest, or an interview question with the digest of the question and its answer — and a **trust label**: `UNTRUSTED_REPOSITORY_CONTENT` for content of the directory or of a repository, `UNTRUSTED_ISSUE_OR_PR_TEXT` for issue and pull-request text, `INTERVIEW_ANSWER` for the person's answer. Admission refuses an attribute without provenance. Every label is untrusted content: nothing the client read or was told becomes authority; it becomes a proposal field with provenance, and only the human acceptance of step 6 makes it canonical.

**Proposal state** is an `Intake` in `CAPTURED` or `PROPOSED`, a `Project` or `Repository` in `PROPOSED`, a `Charter` or `ProjectCharter` revision in `PROPOSED`, a `PlanProposal` in `DRAFT` or `REVIEW`, and a new `WorkBrief`. A revision by the client returns a `PlanProposal` in `REVIEW` to `DRAFT`, and an `Intake` in `PROPOSED` to `CAPTURED` when it revises the `Intake` or any proposal the `Intake` holds.

The intake is staged. The first two stages run in the client and leave nothing in AutoBot; every later one is a resource, so what AutoBot holds is visible, resumable and auditable:
...
2. **Interview.** The client asks the person, one question at a time, for every attribute a required field needs that the directory does not settle, until none is left. Ambiguity is a question, never an assumption: the client never hides it inside a prompt or manufactures precise tasks from an under-specified brief.
3. **Submit.** The client's first submission is the `Intake`. An `Intake` for a project of an existing context lives in that context's namespace. An `Intake` that proposes a new context lives in the installation's intake namespace and carries the proposed `WorkContext` configuration, which is accepted (step 6) before anything else is submitted; that acceptance binds the `Intake` to the new context, and the cross-context rule (TRUST) applies to it from that binding on. The client then proposes the adoption — a new project or an existing one to attach, and the repositories to bind — and records each document the person names and confirms as the brief as an immutable `WorkBrief`: the document's content with its digest, or, for a committed file, a path at a pushed commit of a proposed repository with its content digest. A document beyond the object limits of the profile (M0 §2) is referenced from a pushed commit. A changed document is a new brief revision; the original is never rewritten. Nothing runs and nothing external is created because a proposal was submitted.
4. **Resolve scope.** Repositories and projects are matched by authenticated forge identity, never by names in prose. The Intake controller binds a proposed `Repository` (`ADOPTED`) only when the forge adapter's authenticated answer matches it (§2). It verifies each brief's digest: over the captured content, or through the forge adapter for a path at a commit. A project is never created because a name did not match: a new `Project` stays `PROPOSED` until the plan acceptance of step 6.
5. **Propose.** The `PlanProposal` carries candidate outcomes, milestones, task obligations with the outcome each advances, dependencies, non-goals, acceptance evidence, repository scope, estimated cost, and the consequence class of each task (§5). A `Charter` revision, for a new context, and a `ProjectCharter` revision carry the charter candidates (§7). The Intake controller moves a `PlanProposal` to `REVIEW` once it validates and every reference in it resolves, and the `Intake` to `PROPOSED` once every proposal it holds is valid, every repository it names is `ADOPTED` and every brief digest is verified. The client revises any of them until the person is satisfied.
6. **Accept.** Only an authenticated human principal accepts, through the CLI and never through an intake client: admission refuses every accept command from the intake-submitter identity and from every other principal that is not human. A context that does not exist yet has no reviser, so a proposed `WorkContext` configuration is accepted only by a bootstrap administrator the installation names when AutoBot is installed; accepting it creates the `WorkContext` from it. The configuration names the context's human roles (ROLES §5), and from then on the reviser it names, `WorkContext.spec.revisionAuthority`, accepts the context's charter revisions and plan proposals. Each accept pins the revision and digest of the proposal the human was shown, so a revision the client submits afterwards fails it. An accepted charter revision is a precondition of plan acceptance (§7). Accepting the `PlanProposal` freezes the plan revision — brief digest, bindings, pinned charter revisions, resolved graph, criteria, non-goals, policy, budget, execution profiles — creates the `Plan` in `ACCEPTED` and its `PlanSnapshot`, and moves the proposed projects it binds to `ADOPTED` and the `Intake` to `ACCEPTED`. This is the boundary between planning and execution; activation is KERNEL §5. A human who rejects the `Intake` rejects every proposal it still holds in proposal state.

**Later briefs.** A later brief or document change for an onboarded project takes the same path: a new `Intake` naming the project, a new brief revision, and a `PlanProposal` that names the plan and the revision it revises and proposes a delta, never a re-decomposition. Its human acceptance accepts that revision, which replaces the active one by supersession (KERNEL §5); an accept fails when the plan's active revision is no longer the one the proposal names.
...
Every task carries one **consequence class**, assigned at proposal time and recorded in the plan revision: the proposer — the intake client, or the Manager for a later revision — names a class, and the charter's deterministic path and consequence rules (§7) raise it wherever they require a higher one:
...
**Bootstrap.** Both levels come through the onboarding of §1, as revisions the intake client proposes with every entry a sourced value:

- The context's `Charter` is authored in the interview before the context's first project. Its entries come only from the person's own answers: admission refuses a `Charter` entry whose provenance is not `INTERVIEW_ANSWER`, so no entry is ever extracted.
