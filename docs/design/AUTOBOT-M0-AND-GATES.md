...
- **G-RESTORE** (F-15): isolated restore with witness present, absent and late; old-grant expiry versus active revocation; ambiguous-set reconciliation before dispatch; a restored operation with no send attempt ending `CONFIRMED` without a send by the restored installation when a lookup after the old installation's fence finds it applied, its receipt carrying the remote identity the lookup returned; one whose task is terminal ending `RELEASED` when such a lookup proves it not applied; one on a provider offering neither lookup nor idempotency reaching `UNRESOLVED` with its pair, dispatch refused until it is adjudicated; a restored operation re-requested before the restore, carrying an earlier attempt's record in `prior_send_attempts` and none in `send_attempt`, treated as unsent by every one of these paths and by the barrier, with its ledger entry removed after a currency failure.
