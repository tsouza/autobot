...
- `RepositoryObservation`, `DecompositionRun`, `ReviewRequirement`, `Approval` as kinds — G-INTAKE
- `DecisionPolicy`, fallback kinds (`ABSTAIN`, `DETERMINISTIC_RULE`, `STRONGER_REASONER`), trust-label enum, `DriftAssessment`, judge calibration — G-INTAKE
- Charter import and candidate extraction from a work-in-progress repository, the first charter imported being AutoBot's own repository charter in the six-section format with laws worded generically; the read-only repository projection of the charter; waiver `Decision`s — G-INTAKE
...
- CLI beyond the M0 commands (intake, project adopt, charter propose and accept, plan propose and accept, task start, watch, explain) — G-INTAKE
