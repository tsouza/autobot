...
- **Unsettled operation / `blocked_targets`** — An operation from its `OUTCOME_UNKNOWN` until a terminal state; the bounded `WorkContext` list holding one `(target_identity, operation_uid)` pair per unsettled operation, in the order added, each added before its operation is `OUTCOME_UNKNOWN` and checked in the 2a CAS. Pairs share the ledger's capacity, reserved at acceptance. *(KERNEL §3.3)*
