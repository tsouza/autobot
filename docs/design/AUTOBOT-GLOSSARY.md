...
- **`WorkBrief` / `Intake` / `PlanProposal`** — The immutable record of one brief document, its content or a path at a pushed commit, with its digest; the record of one onboarding submission, which carries a new context's proposed configuration; the candidate graph, revised by the intake client, whose acceptance creates the `Plan` or, when it names a plan, accepts a revision of it. *(ONBOARD §1)*
- **`Project` / `Repository`** — A logical product or initiative that may span repositories; a forge location with its own branch, CI and toolchain. Neither is a plan. A proposed repository is bound only on an authenticated forge answer. *(ONBOARD §1)*
...
- **Intake-submitter identity** — The dedicated principal an intake client authenticates as: it may create and revise intake kinds in proposal state, in the intake namespace and in the namespaces of the contexts it serves, and read their status, nothing else; admission refuses every accept command from it. *(TRUST; ONBOARD §1)*
...
- **Proposal acceptance** — The human act, through the command path, that makes a proposal canonical, pinned to the revision and digest the human was shown: a new context's configuration by a bootstrap administrator, which creates the `WorkContext` and binds its `Intake`; charter revisions and plan proposals by the context's reviser. *(ONBOARD §1)*
