...
- **Dispatch registers** — The `WorkContext` status fields every acceptance is checked against in one CAS: `hold_state`, `hold_generation`, `hold_causes`, `admission_sequence`, `manager_authority[plan]`, `plan_authority[plan]`, `integration_authority[basis]`, `dispatch_authority_generation`, `dispatch_ledger[]`, `blocked_targets[]`, `active_manager_transaction`. *(KERNEL §3.1)*
- **Authority cut** — Any register change: hold transition, Manager install, renewal, drain, epoch advance and resume, plan activation, quiescing, resume and supersession, basis reservation and invalidation, retirement of a plan or basis entry, dispatch-authority advance. All are `WorkContext` CASes and therefore totally ordered with acceptances. *(KERNEL §3.1)*
- **Hold** — The `hold_state` control field of a `WorkContext` (transitions in KERNEL §10); `RequestHold` from `RUNNING` is the cut; `hold_generation` increments on every transition; permits are issued only in `RUNNING`. *(KERNEL §3.2, §10)*
- **`AdmissionStamp` (permit)** — A capability to *request* acceptance for one operation, pinning every register value plus reservation, grant and capsule identities, routing pin, effect digest, nonce and expiry; created by the Context controller on the broker's request. After `ISSUED` its status only acknowledges the ledger and its operation, each transition a Context-applied commit on the broker's command. Never itself authority to send. *(KERNEL §3.2)*
- **`AcceptDispatch`** — The `WorkContext` domain commit, keyed by the permit and submitted by the broker only once the permit's operation records it (`PERMITTED`), that accepts it when every pinned value equals its register and no ledger entry names the permit or its operation; increments `admission_sequence`; appends a ledger entry recording the acceptance. The single linearization point. *(KERNEL §3.2)*
...
- **`send_attempt`** — The write-ahead marker on an operation that precedes opening the remote connection; it belongs to the operation's current `attempt_index`, so present after a crash means "sent or unknown" under the current attempt, and absent means "not sent under the current attempt". A re-request moves it to the append-only `prior_send_attempts`, which keeps every earlier attempt's. *(KERNEL §3.3)*
...
- **`RecoverAcceptedDispatch`** — The restart scan joining ledger and operations by the six-row table, which also brings every permit level with them. *(KERNEL §3.3)*
- **EffectBroker (broker)** — The only path for privileged filesystem, forge, CI, deployment and credential effects; validates grant, scope and capability; requests permits; submits `AcceptDispatch` and the permit acknowledgements; writes `send_attempt`; sends; records the outcome. *(TRUST; KERNEL §3)*
...
- **`ToolInvocation` / `EffectReceipt`** — The same protocol applied to an agent tool call with an effect outside its workspace, its intent carried by an `AgentRun` domain commit and sent under its own permit and acceptance; and the record of its outcome or `OUTCOME_UNKNOWN`. *(KERNEL §3.3)*
...
- **`UNRESOLVED`** — The retained state of a sent operation whose provider can neither deduplicate nor look it up, or of a restored unsent operation whose provider cannot look it up; never garbage-collected, exits only by human adjudication. *(KERNEL §3.3, §7)*
- **Unsent operation / `RELEASED`** — An operation in `REQUESTED`, which carries no `send_attempt`; released without a send, as proven not applied, once no attempt will send it (its intent withdrawn, or its task terminal), or by a human adjudication finding an `UNRESOLVED` one with no `send_attempt` not applied; counts as neither success nor failure. *(KERNEL §3.3)*
- **Carrier / withdrawn intent** — The aggregate whose domain commit carried an operation's intent; a later domain commit of it withdraws the intent, with or without a replacement, by naming its `operation_key` in its input, which its receipt lists in `withdrawn_intents`. *(KERNEL §3.3)*
- **Unsettled operation / `blocked_targets`** — An operation from its `OUTCOME_UNKNOWN`, or, for a restored unsent operation escalated `REQUESTED → UNRESOLVED`, from the pair added before that escalation, until a terminal state, through any re-request after proven non-application; the bounded `WorkContext` list holding one `(target_identity, operation_uid)` pair per unsettled operation, in the order added, each added before its operation is `OUTCOME_UNKNOWN` or escalated, and checked in the 2a CAS. Pairs share the ledger's capacity, reserved at acceptance; only a pair added at recovery row 6 or before that escalation can take the list past the bound. *(KERNEL §3.3, §7)*
- **Dependents (of an unsettled operation)** — Every effect intent with the same `target_identity` whose 2a follows the pair's addition in the CAS order of the `WorkContext` resource, from any attempt, every `IntegrationBasis` whose heads the operation could have changed, and every acceptance evaluation of the owning milestone; all blocked until the operation is terminal. *(KERNEL §3.3)*
- **`COMPENSATED`** — An adjudication outcome of an `UNRESOLVED` operation, recorded only by a human adjudicator with evidence that the effect was compensated outside AutoBot; AutoBot sends no compensating effect in M0. Counts as failure. *(KERNEL §3.3)*
- **Provider capability** — A per-adapter, per-operation declaration (idempotency, lookup, remote marker, head/base requirement, dry run, authoritative rate-limit answers, reconciliation method) with qualification status; an unqualified required semantic yields `BLOCKED_UNSUPPORTED`. *(KERNEL §3.3)*
...
- **`active_manager_transaction` (reservation)** — The one-slot reservation of a short control mutation: reserved, claimed by the target's owning controller before it writes the target (`ClaimManagerTransaction`), and released only with a recorded terminal receipt: the target's, the cancel's or, for a reservation cancelled unclaimed in a drain, the `DrainManager` control receipt. *(KERNEL §4)*
...
- **Rule / waiver** — A waivable guardrail of a charter; an exception to one is a waiver, an `Intervention` answered by a named human (DEFERRED, M0 §5), which no agent grants. *(ONBOARD §7)*
...
- **`Plan` / plan revision / `PlanSnapshot`** — The accepted immutable execution contract; one immutable version of it; the immutable graph bundle of one revision. Only the reviser accepts a plan or a revision. An `ACTIVE` plan is `COMPLETED` when every milestone of its active revision is `ACCEPTED` or `SUPERSEDED` and nothing blocks completion, and ends `FAILED` or `CANCELLED` only on the reviser's `FAIL` or `CANCEL`. *(KERNEL §5, §10; ONBOARD §1, §4; ROLES §5)*
...
- **`RevisionPending` / `PAUSED`** — A Plan *condition* noting an authenticated revision proposal; a plan-level block on new admission requested by an `Intervention` that never weakens a context hold, left only by `RESUME`, `QUIESCE`, `SUPERSEDE`, `FAIL` or `CANCEL`. *(KERNEL §5; ROLES §5)*
...
- **`ScopeCapsule`** — The immutable assignment contract of one `TaskRun`: objective and expected outcome, repositories, path globs, branches, tools, effect kinds, non-goals, acceptance evidence, consequence class, limits, authorizing revisions, the charter digest with the relevant entries of the charter in force, and the capsule digest over all of them. *(ROLES §2)*
...
- **Acceptance suite** — The repository paths of the tests and checks a task's acceptance runs, named in its capsule's acceptance evidence; the worker can neither read nor change it, and the Tester's `VerificationRun`s run it against the pinned candidate. *(ROLES §2)*
- **`Finding`** — A scoped discovery outside the capsule, linked *historically* to a task, and the form of an open question for the Manager; never changes an active capsule. Its severity is classified by a recorded `Decision` of the Decision controller, and its disposition is the Manager's. *(ROLES §2)*
...
- **Tier** — One of the totally ordered capability levels that `WorkContext.spec` policy assigns to every worker-strategy configuration and reviewer configuration; the floor and the review tier of each class are tiers in that order, and admission compares the routing pin's tier against the floor deterministically. *(ONBOARD §5; ROLES §3)*
- **`ExecutionIdentity` / `CredentialGrant`** — The identity each `AgentRun` receives, naming its `TaskRun`, workspace, `AgentRun`, `execution_epoch`, installation and audience, never shared with another `AgentRun` and kept by its continuations; and the short-lived capability bound to it, its audience, repositories, paths, operations, lineage, epoch, expiry and revocation generation. *(KERNEL §6)*
...
- **Drift label** — The vocabulary of a task-local assessment: `ON_TRACK`, `DRIFT_RISK`, `DRIFTED`, `STUCK`, `COMPROMISED`, `INSUFFICIENT_EVIDENCE`; each has a fixed escalation. *(ROLES §1)*
...
- **Custody checkpoint / `CustodyCheckpoint` / `ArtifactCommit` / `Artifact`** — Fence → inventory (including the record outbox) → manifest → upload → verified independent restore → marker → `ArtifactCommit(VERIFIED)` → `PRESERVED`; recurs at the `CustodyPolicy` cadence and at run end, the run lifting the fence with `PRESERVED → IN_USE`; the "next checkpoint" of I-5; the first one `VERIFIED` that recorded an `AgentCheckpoint`'s digest when it began covers that checkpoint. Unknown custody is quarantine. *(KERNEL §7)*
...
- **Reviewer independence** — A reviewer is always a session other than the worker's and never below the review tier of the class; a reviewer on the worker's model is recorded as `correlated`, and a correlated review satisfies required review only for `REVERSIBLE`; required `SECURITY_OR_DATA_INTEGRITY` review comes only from `WorkContext.spec.securityReviewers`. *(ROLES §3)*
...
- **`IntegrationBasis` / `basis_generation`** — The serialized reservation of a milestone's change sets, merge order, heads and the integrated candidate; its generation lives in the register and a merge permit pins it. *(KERNEL §5)*
...
- **Expected-record obligation** — `TaskRun.status.expected_records`: one `outcome` entry for the run and a `usage` entry for each usage producer (a session, or the broker) that spends, committed before it spends, each `PENDING | RECORDED | GAP(uid)` with a `record_deadline` measured from the run's terminal commit, or from the entry's creation when that is later; a record still missing at its deadline becomes a linked gap that counts as unknown outcome or censored cost until a late record closes it. *(KERNEL §8)*
...
- **Typed question / eligible set / `Decision`** — A semantic judgment is a typed question over an eligible set computed deterministically beforehand; the service picks, code decides; the immutable `Decision` records question class, evidence digest, eligible-set digest, selection and policy revision, and is committed only `RECORDED`, by the Decision controller. A `Decision` is never an open question: one for a human is an `Intervention`, one for the Manager a `Finding`. *(THESIS I-8; KERNEL §1)*
...
- **`Intervention`** — An authenticated request — `HOLD`, `RESUME`, `PAUSE`, `QUIESCE`, `SUPERSEDE`, `FAIL`, `CANCEL`, `KILL_SWITCH`, `ADJUDICATE_OPERATION`, `ADJUDICATE_CONFLICT`, `EXCEPTION` — submitted by a human principal, or raised by an owning controller under its own principal to summon one (the answer is then a further human `Intervention` referencing it). A `RESUME` names the `Intervention` it answers, and lifts only that cause of a hold; it is the only exit from a hold and the only way from `PAUSED` back to `ACTIVE`. Applied only through commands to the owning controller: a register or `Plan` CAS for the first eight actions, a CAS on the adjudicated `ExternalOperation` or `WorkspaceConflict` for the two adjudications, and a reference from the `EvidenceBundle` for an `EXCEPTION`; never by editing status. *(ROLES §5)*
- **Human roles** — Reviser (`WorkContext.spec.revisionAuthority`), adjudicator (`adjudicator`), no-test approver (`noTestApprover`), kill-switch operator (`killSwitchOperator`), and the installation's bootstrap administrator; never held by an agent identity or the intake-submitter identity. A `RESUME` of a `KILL_SWITCH` hold comes only from the kill-switch operator, and of any other hold or of a pause only from the reviser. *(ROLES §5)*
...
- **`DriftAssessment`** — The micro-manager's recorded judgment: `ON_TRACK | DRIFT_RISK | DRIFTED | STUCK | COMPROMISED | INSUFFICIENT_EVIDENCE`.
...
- **`intervention_refs` control field** — dropped as a general reference list; an `Intervention` is a kind applied by a command to the owning controller of what it changes, and the one control field that lists `Intervention`s is `hold_causes`, the `HOLD` and `KILL_SWITCH` interventions in force.
