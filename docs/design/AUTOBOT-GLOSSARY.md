...
- **Unsettled operation / `blocked_targets`** — An operation from its `OUTCOME_UNKNOWN` until a terminal state; the bounded `WorkContext` list holding one `(target_identity, operation_uid)` pair per unsettled operation, in the order added and recording the adjudication being applied, each added before its operation is `OUTCOME_UNKNOWN` and checked in the 2a CAS. Pairs share the ledger's capacity, reserved at acceptance. *(KERNEL §3.3)*
...
- **`COMPENSATED`** — The terminal state of an `UNRESOLVED` operation whose adjudication chose compensation and whose separate compensating effect intent, committed by the originating command's owner and sent under the same protocol, reached `CONFIRMED`. Counts as failure. *(KERNEL §3.3)*
