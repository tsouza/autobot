...
- **`AcceptDispatch`** — The `WorkContext` domain commit, keyed by the permit, that accepts it when every pinned value equals its register and no ledger entry names the permit or its operation; increments `admission_sequence`; appends a ledger entry recording the acceptance. The single linearization point. *(KERNEL §3.2)*
