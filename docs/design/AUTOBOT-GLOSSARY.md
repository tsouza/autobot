...
- **`Plan` / plan revision / `PlanSnapshot`** — The accepted immutable execution contract; one immutable version of it; the immutable graph bundle of one revision. Only the reviser accepts a plan or a revision. A plan is `COMPLETED` when every milestone of its active revision is `ACCEPTED` or `SUPERSEDED` and nothing blocks completion, and ends `FAILED` or `CANCELLED` only on the reviser's `FAIL` or `CANCEL`. *(KERNEL §5, §10; ONBOARD §1, §4; ROLES §5)*
...
- **`Intervention`** — An authenticated request — `HOLD`, `RESUME`, `PAUSE`, `QUIESCE`, `SUPERSEDE`, `FAIL`, `CANCEL`, `KILL_SWITCH`, `ADJUDICATE_OPERATION`, `ADJUDICATE_CONFLICT` — submitted by a human principal, or raised by an owning controller under its own principal to summon one (the answer is then a further human `Intervention` referencing it). A `RESUME` names the `Intervention` it answers and lifts only that one; it is the only exit from a hold and the only way from `PAUSED` back to `ACTIVE`. Applied only through commands to the owning controller: a register or `Plan` CAS for the first eight actions, a CAS on the adjudicated `ExternalOperation` or `WorkspaceConflict` for the two adjudications; never by editing status. *(ROLES §5)*
